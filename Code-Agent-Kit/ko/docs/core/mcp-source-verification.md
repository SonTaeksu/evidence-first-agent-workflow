...
## DevExpress — 날짜도 깊이도 다릅니다

위 14개보다 나중인 **2026-08-06**에 추가했고, 같은 표에 넣지 않은 이유는
같은 깊이까지 가지 않았기 때문입니다.

| 서버 | 엔드포인트 | 확인된 것 | 도구 |
|---|---|---|---:|
| `dxdocs` | `https://api.devexpress.com/mcp/docs` | 인증 없이 접속, `tools/list` 응답 | 2 |
| `dxdocs24_2` | `https://api.devexpress.com/mcp/docs?v=24.2` | 없음 — 문서로만 확인, 접속하지 않음 | — |

**실제 `tools/call`은 하지 않았습니다.** 위 14개가 `PASS`인 것은 진짜 호출이
답을 돌려줬기 때문인데, 이 엔드포인트는 거기까지 가지 않았으므로 `PASS`로
적지 않습니다. 고정 URL은 아예 접속하지 않았습니다.

도구 둘 다 입력 스키마가 있고, 이것은 기록해 둘 만한 기계 계약입니다.

- `devexpress_docs_search`는 `technologies`(배열, `minItems: 1`, 항목은 31개
  값의 **닫힌 enum** — `WindowsForms`, `WPF`, `AspNetCore`, `XtraReports`,
  `Blazor`, `VCL`, `XPO`, `eXpressAppFramework`, `Dashboard`,
  `DevExtremeAspNetMvc` 등)와 `question`(문자열)이 **둘 다 필수**입니다.
  산문으로 쓴 플랫폼 이름은 빈 결과가 아니라 스키마 오류입니다.
- `devexpress_docs_get_content`는 `url`이 필수이고, 그 URL은 검색 결과에서
  얻은 것이어야 한다고 스키마 설명이 직접 말합니다. 일반 지식으로 만들면
  안 됩니다.
- 검색 도구 설명은 어떤 `get_content` 호출 앞에도 자신을 먼저 부르라고
  못박습니다. 검색은 발췌만 돌려주기 때문입니다.

### 남겨 둘 정정

이 실행 전에, 공식 문서 밖에서 돌던 스키마(`technologies` + 필수 `question`)를
여기에 *모순됨*으로 적었습니다. 문서에 있는 도구 이름과 맞지 않는다는 근거였는데
**틀렸습니다.** 이름은 처음부터 맞았고, DevExpress가 입력 스키마를 공개하지
않을 뿐이었습니다. 문서에 없다는 것을 문서와 충돌한다고 읽은 것입니다.
**"문서에 없다"와 "문서가 부정한다"는 다릅니다.** 앞의 것은 재보면 닫히는
공백이고, 뒤의 것은 판정인데 그 판정을 뒷받침할 증거가 없었습니다.

### 버전 고정에는 하한이 있습니다

`?v=`는 **v24.2 이상만** 지원합니다. v16.x를 비롯한 24.2 이전 릴리스를 고정하는
공식적인 방법은 없으므로, 그런 프로젝트에서는 문서 서버를 코드에 맞출 수 없고
모든 컨트롤 API 답변이 고정되지 않은 상태가 됩니다.

...
- Theme이 걸린 모든 것에 대해서는 **Publish된 출력**. 실제로 존재하는 Theme Assembly를
  나열한 것입니다. 이 Stack에는 특히 그것이 필요합니다. Compile된 Code가 그것을 참조하지
  않으므로 Build는 그것이 사라진 것을 알아채지 못하기 때문입니다.
...
- Theme이 배포된다는 증거라며 내미는, 개발자 Machine에서 찍은 Screenshot. 그 Machine에는
  DevExpress 전체 설치본이 있고, 대상 Machine에는 Build가 복사한 것만 있습니다. 이 둘은
  서로 다른 주장이며 중요한 것은 두 번째뿐입니다.

...
## 평범한 WPF에는 없고 이 Stack에만 있는 Contract

**저장된 Docking Layout은 이미 설치된 사용자층과 맺은 직렬화된 Contract이며**, 사람들이
그것이 Contract라는 사실 자체를 가장 잘 잊는 대상입니다. 그것은 한 Build가 쓰고 나중의
Build가 읽으므로, Panel의 집합은 Version이 매겨진 Schema입니다. Panel을 없애는 것,
그 정체성을 바꾸는 것, Layout에서 그것을 식별하는 방식을 바꾸는 것은 모두 사용자 Machine에
이미 저장된 모든 Layout에 대한 파괴적 변경입니다. 실패는 양방향으로 조용합니다. 복원된
Layout이 새 Panel을 숨길 수도 있고, 더 이상 없는 Panel을 되살릴 수도 있습니다.

`⟨확인 필요: 이 Project가 Docking Layout을 저장하는지, 거기서 Panel을 무엇으로
식별하는지, 그리고 온전히 적용할 수 없는 Layout을 만났을 때 Application이 어떻게
하는지⟩`

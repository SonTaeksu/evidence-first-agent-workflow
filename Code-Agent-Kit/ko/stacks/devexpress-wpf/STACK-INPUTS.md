...
평범한 WPF 입력은 여기서 반복하지 않습니다. `../csharp-wpf/STACK-INPUTS.md`는 이 Stack의
대안이 아니라 선행 조건입니다. 양쪽 모두 답하십시오.

## Owner 확인

| 입력 | Key | 필수 | 값 또는 경로 | Evidence | 상태 |
|---|---|---:|---|---|---|
| 지원 범위가 아니라 실제로 쓰는 정확한 Version. .NET Target과 Build가 실제로 해석하는 DevExpress Version을 이 Project의 Manifest와 Validation을 실행하는 Machine의 Toolchain에서 읽습니다. | `runtime-sdk-versions` | yes | | | unknown |
| 이 Stack에서 권위 있는 문서 Source는 무엇이고 어떤 것을 참조하면 안 되는지. mcp/source-routing.md에 기록합니다. | `authoritative-sources` | yes | | | unknown |
| Build가 어느 DevExpress Feed에서 복원하는지, Build Machine에서 그 인증이 어떻게 되는지, 그리고 Licence가 이 Project의 사용을 포함하는지. DevExpress Package를 복원하지 못하는 Build는 Build가 아니며, 이것은 Repository에서 도출되지 않습니다. | `devexpress-license-and-feed` | yes | | | unknown |
...
| 이 Stack의 자료를 공개 Repository에 둘 수 있는지 여부. DevExpress 자료에는 Licence가 걸려 있으므로, 여기서는 Open Source Stack에서보다 더 좁은 질문입니다. | `confidentiality` | yes | | | unknown |
...
| 해석된 DevExpress Version | Build의 복원 출력에서 읽음 | | |
| Publish된 출력의 Theme Assembly | Project File이 아니라 Build 출력에서 읽음 | | |

...
- Build (if any) and test: the configured scripts. Exit code is the verdict.
- Record `node --version` from the machine that ran them; the manifest range is not evidence.

...
두 File이 어긋난 채 `ready`를 선언하는 Stack은 실패합니다. `Key` 열이 없는 표는 불일치보다
더 나쁩니다. 아예 비교되지 않으며, 그 사실 또한 보고됩니다. 양쪽을 모두 채우거나, Interview
Prompt(`prompts/8-fill-stack.md`)를 실행해 Agent가 둘을 맞춰 두게 합니다.

순수 Windows Forms 입력은 여기서 반복하지 않습니다.
[`../csharp-winforms/STACK-INPUTS.md`](../csharp-winforms/STACK-INPUTS.md)도 함께
답변합니다 — Project Format, Package 관리, Data Access, Authentication, Localization,
Deployment는 DevExpress Project에서도 여전히 열린 질문입니다.

## Owner 확인

| 입력 | Key | 필수 | 값 또는 경로 | Evidence | 상태 |
|---|---|---:|---|---|---|
| 실제로 쓰는 정확한 Version — 지원 범위가 아닙니다. .NET Target은 Project 자체 Manifest에서 읽고, DevExpress Release는 Package 또는 Assembly 참조와 Validation을 실행하는 Machine에서 읽습니다. | `runtime-sdk-versions` | yes | | | unknown |
...
| DevExpress License가 Build하는 Machine에 어떻게 도달하는지, licenses.licx가 Source Control에 들어 있는지, 그리고 Assembly를 Application과 함께 재배포해도 되는지. | `licensing` | yes | | | unknown |
| 이 Stack의 자료를 공개 Repository에 둘 수 있는지 여부 — Form Layout, Skin 이름, Screenshot을 포함해서. | `confidentiality` | yes | | | unknown |
...
|---|---|---|---|
| Project 및 Package Manifest | Repository에서 읽음 | | |
| DevExpress 참조와 그 Version | Project File과 Lock 또는 Package File에서 읽음 | | |
| 설치된 DevExpress Assembly | 검증을 실행하는 Machine에서 읽음 | | |
| Designer File과 그것이 생성하는 DevExpress Type | Repository에서 읽음 | | |
| `licenses.licx`와 그것이 추적되고 있는지 여부 | Repository에서 읽음 | | |

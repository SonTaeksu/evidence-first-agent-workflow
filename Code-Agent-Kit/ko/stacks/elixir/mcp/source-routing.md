...
|---|---|---|---:|---|
| `elixir-docs` | `https://gitmcp.io/elixir-lang/elixir` | **PASS** | 4 | 공식 언어 Repository |
...
용도: 언어 자체와 그 표준 Library. Phoenix, LiveView, Ecto는 별개의 Project이며 여기서 다루지 않습니다.
...
아래는 Local `stdio` Server입니다. 확인 실행에 **포함되지 않았으므로**, 이것들이
동작한다고 말하는 대목은 여기에 없습니다. 아무도 다시 찾아 헤매지 않도록 이름을
적어 두었고, 이름을 검사로 착각하지 않도록 표시해 두었습니다.

**`hexdocs-mcp`** — `npx -y hexdocs-mcp@0.5.0`

Hex Package 문서에 대한 의미 기반 검색. 확인 실행에 없었고, Local에 Node 22, Elixir, OTP, Ollama가 필요하며, 자체 문서는 v0.6.0을 서술하는데 안정 Release는 v0.5.0입니다.

...
|---|---|---|---:|---|
| `rust-book` | `https://gitmcp.io/rust-lang/book` | **PASS** | 4 | official book |
| `rust-reference` | `https://gitmcp.io/rust-lang/reference` | **PASS** | 4 | official reference |
...
Endpoint: `https://gitmcp.io/rust-lang/book`  (Streamable HTTP)

Tools actually exposed, as observed rather than as documented:

- `fetch_book_documentation`
- `search_book_documentation`
- `search_book_code`
- `fetch_generic_url_content`

Use it for: concepts and idiom — ownership, borrowing, error handling.

### `rust-reference` — PASS

Endpoint: `https://gitmcp.io/rust-lang/reference`  (Streamable HTTP)

Tools actually exposed, as observed rather than as documented:

- `fetch_reference_documentation`
- `search_reference_documentation`
- `search_reference_code`
- `fetch_generic_url_content`

Use it for: the exact language rule — syntax, types, expressions, lifetime and trait-object bounds. The book explains; the reference decides.
...
```json
{
  "mcpServers": {
    "rust-book": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "https://gitmcp.io/rust-lang/book"]
    }
  }
}
```
...
```bash
npx -y @modelcontextprotocol/inspector --cli https://gitmcp.io/rust-lang/book   --transport http --method tools/list --format json
```
